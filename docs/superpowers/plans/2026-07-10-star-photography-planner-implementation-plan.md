# 별 사진 촬영지 추천 서비스 1차 구현 계획

## 목표

1차 구현의 목표는 전 세계 전체 기능을 한 번에 완성하는 것이 아니라, 대표 질문 하나가 처음부터 끝까지 동작하는 MVP 세로 조각을 만드는 것이다.
...
- 디자인 디테일은 후속 디자인 단계에서 다룬다. 이번 단계에서는 사용 흐름과 상태 동기화가 더 중요하다.
