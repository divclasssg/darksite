...
Darksite는 전 세계를 대상으로 별 사진 촬영에 가장 좋은 날짜와 장소를 추천하는 반응형 웹 앱이다. 이름은 천체사진가들이 찾는 어두운 촬영지인 `dark site` 개념에서 온다. 이 서비스는 달 조건, 날씨, 어두운 하늘 조건, 실제 밤의 길이, 선택적인 은하수 적합도를 함께 사용해 다음과 같은 계획 질문에 답한다.
...
- 프론트엔드와 배포는 Next.js + Vercel 조합을 사용한다.
- 첫 빌드는 로컬 fixture를 우선 사용하되, 실제 API 키가 필요한 데이터 제공자는 Next.js Route Handler를 통해 서버 측에서 호출할 수 있게 설계한다.
